      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Measure and visualize MPEG-TS signal loudness levels to normalize content and reduce channel switching discrepancies.
          </p>
        </div>
        ...
        {/* Legend */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 text-center">Loudness Level Color Coding</h3>
          <div className="flex flex-wrap justify-center gap-8">
            <div className="flex items-center">
              <div className="w-6 h-6 bg-green-500 rounded mr-3"></div>
              <span className="text-gray-700">Normal/Acceptable (-23.0 to -22.0 dB)</span>
            </div>
            <div className="flex items-center">
        ...
        {!loading && !error && heatmapData && heatmapData.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Detailed Data Table</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">Channel Name</th>
                    <th className="border border-gray-300 px-4 py-3 text-left font-semibold text-gray-700">Multicast IP</th>
                    {heatmapData[0] && Object.keys(heatmapData[0].readings).sort().map(timeSlot => (
                      <th key={timeSlot} className="border border-gray-300 px-4 py-3 text-center font-semibold text-gray-700">
                        {timeSlot}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {heatmapData.map((stream, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      <td className="border border-gray-300 px-4 py-3 font-medium text-gray-900">{stream.channelName}</td>
                      <td className="border border-gray-300 px-4 py-3 text-blue-600 underline cursor-pointer">{stream.ip}</td>
        ...
        {!loading && !error && (!heatmapData || heatmapData.length === 0) && (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-lg text-gray-600">No data available for the selected date.</p>
          </div>
        )}
      </div>
    </div>
